...
export default function StudReadComp(){
    ...
    const DeleteStud = (ParMobile)=>{
        let NewList = user.filter((per)=>(per.ParentMobile !== ParMobile))
        setUser(NewList)
    }
                      ...
                      <tbody>    
                        {PageData.map((stu,index)=>(
                            <tr key={index}>
                                ...
                                <td><button onClick={()=>DeleteStud(stu.ParentMobile)} className="delete-btn">Delete</button></td>
                            </tr>
                        ))}    
                            <tr>
